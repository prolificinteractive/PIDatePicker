...
public class PIDatePicker: UIControl, UIPickerViewDataSource, UIPickerViewDelegate {
    ...
    */
    private func refreshComponentOrdering() {
        var componentOrdering = DateFormatter.dateFormat(fromTemplate: "yMMMMd", options: 0, locale: self.locale)!
        ...
        let characterSet = CharacterSet(charactersIn: String(firstComponentOrderingString) + String(lastComponentOrderingString))
        componentOrdering = componentOrdering.trimmingCharacters(in: characterSet).trimmingCharacters(in: CharacterSet.whitespacesAndNewlines)
